...
abstract public class AbstractSubmitter implements Submitter {
    ...
    protected String wrapperScripts[] = {"oge_job_script.sh","oge_job_script_legacy.sh","common.sh"}; //default is OGE script for aligners and analyses
    ...
    protected void completeJobEnvironment(ExecutableJob job, String jobDir) {
        ...
        environment.put("FILESET_COMMAND",
                String.format("java ${PLUGIN_NEED_DEFAULT_JVM_OPTIONS} -cp ${RESOURCES_GOBYWEB_SERVER_SIDE_DEPENDENCIES_JAR}:${RESOURCES_GOBYWEB_SERVER_SIDE_FILESET_JAR} -Dlog4j.configuration=file:${RESOURCES_GOBYWEB_SERVER_SIDE_LOG4J_PROPERTIES} org.campagnelab.gobyweb.filesets.JobInterface --fileset-area-cache ${FILESET_TARGET_DIR} --pb-file %s/filesets.pb --aggregated-metadata-file %s/aggregated-metadata-file.pb --job-tag %s ",
                        jobDir,  jobDir,
                        job.getTag(),
